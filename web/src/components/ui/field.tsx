...
import { localizeErrorMessage } from "@/lib/api/errorMessages";
  ...
  const content = useMemo(() => {
    ...
    const uniqueErrors = [
      ...new Map(
        errors.map((error) => [
          error?.message,
          error?.message ? localizeErrorMessage(error.message) : undefined,
        ])
      ).values(),
    ];
    ...
    return (
      <ul className="ml-4 flex list-disc flex-col gap-1">
        {uniqueErrors.map((message, index) => message && <li key={index}>{message}</li>)}
      </ul>
    );
  }, [children, errors]);
