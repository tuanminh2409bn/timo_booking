...
import { updateEmployeePasswordSchema } from "./employee-shared.js";
...
export const updateEmployeePassword = async (req: Request, res: Response) => {
  ...
  const passwordParseResult = updateEmployeePasswordSchema.safeParse(req.body);

  if (!passwordParseResult.success) {
    return createErrorResponse(res, SERVICE_ERRORS.invalidRequest, {
      validation: passwordParseResult.error.flatten().fieldErrors,
    });
  }
  ...
  await firebaseAuthRepository.auth.updateUserPassword(
    ...
    passwordParseResult.data.password,
  );
  await firebaseAuthRepository.auth.revokeRefreshTokens(existingEmployee.uid);
