...
export const blockCustomer = async (request: Request, response: Response) => {
  ...
  if (!storeId || !customerId || !parsed.success) {
  ...
  try {
    const customer = await firestoreRepository.shop.customer.blockShopCustomer(
      authContext.ownerId,
      storeId,
      customerId,
      {
        ...
        role: authContext.role === "manager" ? "manager" : "owner",
      },
    );
